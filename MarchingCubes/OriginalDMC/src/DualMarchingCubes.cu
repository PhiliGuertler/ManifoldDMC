...
__global__ void quadrilateral_to_triangle(Quadrilaterals q_, Vertices v_, Triangles t_)
{
    ...
    t_.addTriangle(2 * tid, v0, v1, v3);
    t_.addTriangle(2 * tid + 1, v3, v2, v0);
    /*
...
__global__ void map_quadrilaterals(QuadrilateralHashTable ht_, Quadrilaterals q_)
{
    ...
    int4 quad = ht_.quadrilateral(tid);
    //printf("Quad: [%d, %d, %d, %d]\n", quad.x, quad.y, quad.z, quad.w);
    //printf("%4d Quad: [%d, %d, %d, %d]\n", tid, quad.x, quad.y, quad.z, quad.w);
    q_.addColoredQuadrilateral(quad, ht_.color(tid));
}
...
p_mc::DualMarchingCubes::dualMC(const float i0, Mesh& mesh, std::map<std::string,int>& config)
{
    ...
    std::cout << "[DEBUG] numCells: " << t_size << std::endl;
